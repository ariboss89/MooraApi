...
namespace MooraMethodApi.Controllers
{
    public class ConfigController : ApiController
    {
        ...
        public string IdKeputusan()
        {
            ...
            {
                _connection.Open();
                MySqlCommand _command = new MySqlCommand("SELECT *FROM tb_config WHERE config_key = 'keputusan'", _connection);
        ...
        [HttpPost]
        public void UpdateConfigValue(tb_config con)
        {
            using (MySqlConnection _connection = new MySqlConnection(connStr))
            {
                MySqlCommand sqlCmd = new MySqlCommand();
                sqlCmd.CommandType = CommandType.Text;
                sqlCmd.CommandText = "UPDATE tb_config SET config_value = @value WHERE config_key= @key";
